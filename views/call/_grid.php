...
if($client != null) {
    ...
    $columns[] = [
        ...
        'label' => "Откуда >>> Куда",
        'content' => function($model) use($canceled_order_status_id, $user_role_alias) {
...
//                .intval($model->suitcase_count).'Ч, '.intval($model->bag_count).'С, '.intval($model->oversized_count).'Н'; // чемоданы, сумки, негабариты
            ...
            return
                $text_from.' >>> '.$text_to
                .'<hr style="margin: 5px 0; border: none; background-color: #000000; color: #000000; height: 1px;" />'
                //.$str.'<br />ВПЗ: '.$vpz
                .$str.'<br />'
                //.'<hr style="margin: 5px 0; border: none; background-color: #000000; color: #000000; height: 1px;" />'
                .$places_count.'М, '.intval($model->child_count).'Д, '.intval($model->student_count).'С '
                .'<b>БЖ</b>: '.intval($model->suitcase_count).'Ч, '.intval($model->bag_count).'С, '.intval($model->oversized_count).'Н';
    ...
    $columns[] = [
        ...
        'content' => function ($model) use($canceled_order_status_id) {
            ...
            $html = EditableTextWidget::widget([
                'name' => 'time_confirm',
...
//    ];
    ...
    $columns[] = [
        ...
        'content' => function($model) use($canceled_order_status_id)
        {
            ...
            $options['open_trip_page'] = true;
            $options['trip_id'] = $model->trip_id;

            $html = PopupFormWidget::widget([
            ...
            if($model->confirm_selected_transport == 1) {
                $html .= '<br /><span class="center glyphicon glyphicon-ok-sign btn-xs_" style="margin: 4px 4px; font-size: 16px;"></span>';
            }else {
                if(!empty($model->time_confirm) && $model->is_confirmed == 1 && $model->fact_trip_transport_id > 0) {
                    $html .= '<br /><button class="btn btn-sm_ btn-warning confirm-selected-transport" type="button" title="Клиент знает машину ?"><i class="glyphicon glyphicon-question-sign"></i></button>';
                }else {
                    $html .= '<br /><button class="btn btn-sm_ btn-warning confirm-selected-transport" disabled="disabled" title="Выберите ВРПТ, подтвердите ВРПТ и выберите факт.т/с" type="button"><i class="glyphicon glyphicon-question-sign"></i></button>';
                }
            }
